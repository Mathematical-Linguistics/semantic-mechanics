...
\vspace{0.35em}\noindent
\begin{tikzpicture}
\node[draw=primaryblue, fill=calloutbg, rounded corners=4pt, line width=0.9pt, text width=\dimexpr\linewidth-24pt\relax, inner sep=6pt] {
...
\vspace{0.35em}\noindent
\begin{tikzpicture}
\node[draw=accentindigo, fill=warmbg, rounded corners=4pt, line width=0.9pt, text width=\dimexpr\linewidth-24pt\relax, inner sep=6pt] {
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[every node/.style={font=\small\sffamily}, scale=0.82]
    % Source Sequence Box
    \draw[fill=calloutbg, draw=primaryblue, line width=1.1pt, rounded corners=10pt] (-7.9,-1.65) rectangle (-3.0,1.65);
    \node[color=primaryblue, font=\bfseries\small] at (-5.45, 1.12) {Source Sequence $\Sigma_A^*$};
    \node[font=\footnotesize] at (-5.45, 0.50) {String: $\mathbf{s}_A = (s_1, \dots, s_n)$};
    \node[font=\scriptsize, color=darkslate!85] at (-5.45, -0.10) {Alphabet Cardinality $|\Sigma_A| = M_A$};
    \node[font=\scriptsize, color=accentindigo] at (-5.45, -0.82) {Entropy: $H(\Sigma_A) = -\sum p_i \log_2 p_i$};

    % Transformation Engine Middle Box
    \draw[fill=warmbg, draw=accentteal, line width=1.2pt, rounded corners=10pt] (-2.2,-1.65) rectangle (2.2,1.65);
    \node[color=accentteal, font=\bfseries\small] at (0, 1.18) {Sequence Engine $\mathcal{E}$};
    \node[font=\footnotesize\bfseries, color=darkslate] at (0, 0.74) {Mechanical Work $\mathcal{W}_{\text{ext}}$};
    \node[font=\scriptsize, color=darkslate!85] at (0, 0.34) {Stroke Deformation};
    \node[font=\footnotesize, color=darkslate!90] at (0, -0.08) {$\int \kappa(s)^2\,ds$};
    \draw[->, line width=1.1pt, color=red!75!black, decorate, decoration={snake, amplitude=1.5pt, segment length=5pt}] (0, -0.28) -- (0, -0.62);
    \node[font=\scriptsize\bfseries, color=red!75!black] at (0, -1.02) {Heat $\mathcal{Q} \ge k_B T \ln 2 \cdot \Delta H$};

    % Inter-box Arrows
    \draw[->, line width=1.3pt, color=primaryblue] (-2.92, 0.0) -- (-2.28, 0.0) node[midway, above=2pt, font=\scriptsize\bfseries] {$\mathbf{s}_A$};
    \draw[->, line width=1.3pt, color=primaryblue] (2.28, 0.0) -- (2.92, 0.0) node[midway, above=2pt, font=\scriptsize\bfseries] {$\mathbf{s}_B$};

    % Target Sequence Box
    \draw[fill=calloutbg, draw=primaryblue, line width=1.1pt, rounded corners=10pt] (3.0,-1.65) rectangle (7.9,1.65);
    \node[color=primaryblue, font=\bfseries\small] at (5.45, 1.12) {Target Sequence $\Sigma_B^*$};
    \node[font=\footnotesize] at (5.45, 0.50) {String: $\mathbf{s}_B = (s_1^\prime, \dots, s_m^\prime)$};
    \node[font=\scriptsize, color=darkslate!85] at (5.45, -0.10) {Alphabet Cardinality $|\Sigma_B| = M_B$};
    \node[font=\scriptsize, color=accentindigo] at (5.45, -0.82) {Entropy: $H(\Sigma_B) = -\sum q_j \log_2 q_j$};

    % Bottom Invariant Box
    \node[draw=boxborder, fill=white, rounded corners=4pt, line width=0.8pt, inner sep=4.5pt] at (0,-2.38) {
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.93\textwidth]{figures/fig_glyph_comparative_topology.png}
\caption{Multi-Script Glyph Topology and Differential Curvature Decomposition: comparative boundary discretization and topological analysis of Latin, Arabic, Hangul, and Hanzi scripts. Discrete sample points along each vector boundary trace arc-length parameterization $s$. Phonetic systems minimize stroke work $\mathcal{E}_{\text{stroke}}$ to accelerate physical production, while logographic scripts maximize single-character informational payload $H(s)$ through complex differential curvature.}
\label{fig:glyph_topology}
\end{figure}
...
The computational laboratory implements numerical stroke curvature evaluation and theoretical Landauer dissipation limits:

\begin{lstlisting}[language=Python, basicstyle=\ttfamily\scriptsize, caption={Curvature energy and Landauer dissipation in sequence translation.}, label={lst:thermo}]
import numpy as np

def compute_stroke_curvature_energy(points: np.ndarray, alpha: float = 1.0, beta: float = 2.0) -> float:
    # 1. First-order tangent arc length ds
    dx, dy = np.gradient(points[:, 0]), np.gradient(points[:, 1])
    ds = np.sqrt(dx**2 + dy**2) + 1e-8
    # 2. Second-order curvature acceleration
    ddx, ddy = np.gradient(dx), np.gradient(dy)
    # 3. Signed curvature kappa and bending action
    curvature = np.abs(dx * ddy - dy * ddx) / (ds**3)
    return float(np.sum((alpha + beta * (curvature**2)) * ds))

def landauer_dissipation(entropy_erased_bits: float, temp_kelvin: float = 300.0) -> float:
    k_B = 1.380649e-23  # Boltzmann constant (J/K)
    return float(k_B * temp_kelvin * np.log(2) * entropy_erased_bits)
\end{lstlisting}
...
\begin{enumerate}[noitemsep,topsep=1pt,itemsep=1.5pt]
    \item \textbf{Thermodynamic Efficiency of Natural Alphabets:} Why did Phoenician, Greek, and Latin stabilize at 20--30 phonetic symbols rather than 10,000 ideograms? What thermodynamic trade-off between stroke work and sequence length does this optimize?
    \item \textbf{Reversible Computing and Lossless Translation:} If formal translation is a category-theoretic isomorphism ($F \circ G \cong \mathbf{1}$), can it be implemented on reversible hardware with zero Landauer heat dissipation?
    \item \textbf{The Energy Cost of Hallucination:} In large language models, generating divergent tokens increases sequence entropy. How does this entropy generation manifest in physical power draw across GPU clusters?
    \item \textbf{Typographic Wear and Historical Media:} How did the physical wear of lead type and mechanical punch-cutting constrain the evolution of European letterforms (e.g., Blackletter vs. Antiqua)?
\end{enumerate}
}
