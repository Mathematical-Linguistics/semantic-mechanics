...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[every node/.style={font=\small\sffamily}, scale=0.83]
    % Source Sequence Box
    ...
    \node[color=accentteal, font=\bfseries\small] at (0, 1.14) {Sequence Engine $\mathcal{E}$};
    \node[font=\footnotesize] at (0, 0.72) {Mechanical Work $\mathcal{W}_{\text{ext}}$};
    \node[font=\scriptsize, color=darkslate] at (0, 0.36) {Stroke Deformation};
    \node[font=\footnotesize, color=darkslate!85] at (0, -0.12) {$\int \kappa(s)^2\,ds$};
    \draw[->, line width=1.1pt, color=red!75!black, decorate, decoration={snake, amplitude=1.5pt, segment length=5pt}] (0, -0.38) -- (0, -0.74);
    \node[font=\scriptsize\bfseries, color=red!75!black] at (0, -1.04) {Heat $\mathcal{Q} \ge k_B T \ln 2 \cdot \Delta H$};
