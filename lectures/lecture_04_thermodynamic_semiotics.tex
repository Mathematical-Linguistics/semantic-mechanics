...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[every node/.style={font=\small\sffamily}, scale=0.85]
    % Source Sequence Box
    \draw[fill=calloutbg, draw=primaryblue, line width=1.1pt, rounded corners=12pt] (-7.6,-1.35) rectangle (-2.8,1.35);
    \node[color=primaryblue, font=\bfseries\small] at (-5.2, 0.95) {Source Sequence $\Sigma_A^*$};
    \node[font=\footnotesize] at (-5.2, 0.4) {String: $\mathbf{s}_A = (s_1, \dots, s_n)$};
    \node[font=\scriptsize, color=darkslate!80] at (-5.2, -0.15) {Alphabet Cardinality $|\Sigma_A| = M_A$};
    \node[font=\scriptsize, color=accentindigo] at (-5.2, -0.65) {Entropy: $H(\Sigma_A) = -\sum p_i \log_2 p_i$};

    % Transformation Engine Middle
    \draw[fill=warmbg, draw=accentteal, line width=1.2pt, rounded corners=12pt] (-1.6,-1.35) rectangle (1.6,1.35);
    \node[color=accentteal, font=\bfseries\small] at (0, 0.95) {Sequence Engine $\mathcal{E}$};
    \node[font=\scriptsize] at (0, 0.5) {Mechanical Work $\mathcal{W}_{\text{ext}}$};
    \node[font=\tiny, color=darkslate] at (0, 0.1) {Stroke Deformation};
    \node[font=\tiny, color=darkslate!75] at (0, -0.15) {$\int \kappa(s)^2 ds$};
    \draw[->, line width=1.2pt, color=red!75!black, decorate, decoration={snake, amplitude=1.8pt, segment length=5pt}] (0, -0.32) -- (0, -0.85);
    \node[below, font=\tiny, color=red!75!black] at (0, -0.87) {Heat $\mathcal{Q} \ge k_B T \ln 2 \cdot \Delta H$};

    % Inter-box Arrows
    \draw[->, line width=1.3pt, color=primaryblue] (-2.6, 0.1) -- (-1.8, 0.1) node[midway, above, font=\scriptsize] {$\mathbf{s}_A$};
    \draw[->, line width=1.3pt, color=primaryblue] (1.8, 0.1) -- (2.6, 0.1) node[midway, above, font=\scriptsize] {$\mathbf{s}_B$};

    % Target Sequence Box
    \draw[fill=calloutbg, draw=primaryblue, line width=1.1pt, rounded corners=12pt] (2.8,-1.35) rectangle (7.6,1.35);
    \node[color=primaryblue, font=\bfseries\small] at (5.2, 0.95) {Target Sequence $\Sigma_B^*$};
    \node[font=\footnotesize] at (5.2, 0.4) {String: $\mathbf{s}_B = (s_1^\prime, \dots, s_m^\prime)$};
    \node[font=\scriptsize, color=darkslate!80] at (5.2, -0.15) {Alphabet Cardinality $|\Sigma_B| = M_B$};
    \node[font=\scriptsize, color=accentindigo] at (5.2, -0.65) {Entropy: $H(\Sigma_B) = -\sum q_j \log_2 q_j$};

    % Bottom Invariant Box
    \node[draw=boxborder, fill=white, rounded corners=4pt, line width=0.8pt, inner sep=4pt] at (0,-2.05) {
        \footnotesize{\bfseries Jarzynski Identity:} $\left\langle \exp\left(-\frac{\mathcal{W}}{k_B T}\right) \right\rangle = \exp\left(-\frac{\Delta F}{k_B T}\right) \quad \text{where } \Delta F = F(\Sigma_B) - F(\Sigma_A)$
    };
\end{tikzpicture}
